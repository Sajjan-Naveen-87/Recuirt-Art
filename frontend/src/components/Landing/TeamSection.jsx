...
function TeamSection() {
  ...
  const fallbackData = [
    { name: "Sahitya Saini", image: "/Founder-Image.jpeg", linkedin_url: "https://www.linkedin.com/in/sahitya-saini-7b104a98" },
    { name: "Mansi Agrawal", image: "/Mansi-Agrawal.jpeg", linkedin_url: "https://www.linkedin.com/in/mansi-agrawal-034b041b5" },
    { name: "Prerana Agrawal", image: "/Prerana-Agrawal.png", linkedin_url: "https://www.linkedin.com/in/prerna-agrawal-24b87b135" },
    { name: "Kratika Singh", image: "/Kratika-Singh.jpeg", linkedin_url: "https://www.linkedin.com/in/kratika-singh-591b0a231" }
  ];
  ...
  const fetchTeamMembers = async () => {
    try {
      setLoading(true);
      const response = await feedbackService.getTeamMembers();
  ...
  if (loading && team.length === 0) {
    return (
      <div className="py-20 flex justify-center items-center">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#cbd5b1]"></div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 md:gap-12 lg:gap-20">
          {team.map((member, index) => {
            const fallback = fallbackData.find(f => f.name === member.name);
            const imageUrl = member.image 
              ? (member.image.startsWith('http') ? member.image : `${BACKEND_URL}${member.image}`)
              : (fallback ? fallback.image : '/Founder-Image.jpeg');

            return (
              <a 
                key={index} 
                href={member.linkedin_url || (fallback ? fallback.linkedin_url : '#')}
                target="_blank"
